...
W1 = np.random.rand(H,dx)*.001
W2 = np.random.rand(C,H)*.001
b1 = np.ones((H,1))
b2 = np.ones((C,1))
...
#while ITER < 2000:
    for ii in range(0,cyc):
        ...
        A2_Training_mb = P(Z2_Training_mb)        

        #Update Gradient Descent Weights
        dZ2 = A2_Training_mb - Y_Training_1Hot[:,128*ii:128*(ii+1)]
    ...
    Y_NN_Testing = np.argmax(A2_Testing, axis=0)
    ...
    Y_NN_Validation = np.argmax(A2_Validation, axis=0)
